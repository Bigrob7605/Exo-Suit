...
function Test-FileShouldScan {
    ...
    if ($fileSizeMB -gt $MaxFileSizeMB) {
        Write-Log " Skipping large file: $($File.Name) ($fileSizeMB MB)" -Level Verbose -Color Yellow
        return $false
    }
    ...
    if ($SkipBinary) {
        try {
            ...
            if ($sample -contains 0) {
                Write-Log " Skipping binary file: $($File.Name)" -Level Verbose -Color Yellow
                return $false
            }
        }
        catch {
            Write-Log " Could not read file for binary detection: $($File.Name)" -Level Verbose -Color Yellow
        }
    }
...
function Scan-FileForEmojis {
    ...
    try {
        ...
        Write-Log " Error scanning file $FilePath : $_" -Level Warning -Color Yellow
        return @()
    }
}
