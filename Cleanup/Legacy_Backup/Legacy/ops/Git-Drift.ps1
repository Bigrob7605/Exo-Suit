...
function Invoke-GitCommand {
    ...
    try   { & git -C $Path @($Command.Split(' ')) 2>$null }
    catch { Write-Warning "git $Command failed"; $null }
}

function Out-DriftReport {
    param(
        [string]$Dir = (Join-Path $Path '.git-drift'),
        [array]$Data
    )
...
if (-not (Test-Path (Join-Path $Path '.git'))) {
    Write-Host "Not in a git repository at $Path  skipping drift detection." -ForegroundColor Yellow
    return
}
