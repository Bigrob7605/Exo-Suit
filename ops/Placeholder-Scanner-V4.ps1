...
function Get-SupportedFiles {
    ...
    try {
        ...
        Write-PlaceholderScannerLog "Extensions: $($extensions -join ', ')" "INFO"
        Write-PlaceholderScannerLog "Search pattern: $searchPattern" "INFO"
        
        # Get files for each extension separately to avoid pattern issues
        $files = @()
        foreach ($ext in $extensions) {
            $pattern = "*$ext"
            $extFiles = Get-ChildItem -Path $Path -Recurse -Filter $pattern -File -ErrorAction SilentlyContinue
            $files += $extFiles
        }
