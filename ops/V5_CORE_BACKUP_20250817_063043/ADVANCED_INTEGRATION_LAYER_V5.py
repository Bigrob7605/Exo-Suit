...
Health Check: {'EMOJI_2705 PASS' if alert_data['health_check'].get('success') else 'EMOJI_274C FAIL'}
Fire Drill: {'EMOJI_2705 PASS' if alert_data['fire_drill'].get('success') else 'EMOJI_274C FAIL'}
Critical Files: {'EMOJI_2705 ALL PRESENT' if all(alert_data['critical_files'].values()) else 'EMOJI_274C MISSING'}
...
"""
    ...
    def monitor_repositories(self):
        ...
        logging.info("EMOJI_1F3DB️ Kai Sentinel Mesh started - monitoring repositories")
    ...
    def start_monitoring(self):
        ...
        logging.info("EMOJI_1F3DB️ Kai Sentinel Mesh monitoring started")
    ...
    def stop_monitoring(self):
        ...
        logging.info("EMOJI_1F3DB️ Kai Sentinel Mesh monitoring stopped")
        
    def run_single_check(self):
        """Run a single check of all repositories."""
        logging.info("EMOJI_1F3DB️ Running single check of all repositories")
...
def main():
    ...
    if args.start:
        print("EMOJI_1F3DB️ Starting Kai Sentinel Mesh monitoring...")
        ...
        try:
            ...
            print("\nEMOJI_1F3DB️ Stopping Kai Sentinel Mesh...")
            sentinel.stop_monitoring()
            
    elif args.check:
        print("EMOJI_1F3DB️ Running single check...")
        results = sentinel.run_single_check()
        
        for result in results:
            status_emoji = "EMOJI_2705" if result['status'] == 'HEALTHY' else "EMOJI_274C"
            print(f"{status_emoji} {result['repo_name']}: {result['status']}")
            
            if result['alerts']:
                for alert in result['alerts']:
                    print(f"  EMOJI_26A0️  {alert}")
