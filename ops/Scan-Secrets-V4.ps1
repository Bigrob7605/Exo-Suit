    ...
    try {
        ...
        foreach ($rule in $Rules) {
            try {
                ...
                Write-Log " Error processing rule $($rule.Name) in $($File.Name): $_" -Color Yellow
            }
        }
    }
    catch {
        Write-Log " Error scanning file $($File.Name): $_" -Color Yellow
    }
...
function Export-SARIFReport {
    ...
    Write-Log " Generating SARIF report..." -Color Cyan
...
function Start-SecretScan {
    ...
    Write-Log " Starting V4.0 Ultra-robust Secret Scan..." -Color Cyan
    Write-Log " Root directory: $RootPath" -Color Cyan
    ...
    Write-Log " Found $($files.Count) files to scan" -Color Cyan
    ...
    if ($Parallel -and $files.Count -gt 10) {
        ...
        foreach ($file in $files) {
            ...
            if ($null -ne $findings -and $findings.Count -gt 0) {
                ...
                if ($PSCmdlet.MyInvocation.BoundParameters['Verbose']) {
                    Write-Log " Found $($findings.Count) secrets in: $($file.Name)" -Color Gray
                }
            }
        }
    }
    ...
    # Display results
    Write-Log "`n Secret Scan Complete!" -Color Cyan
    Write-Log " Duration: $duration seconds" -Color Cyan
    Write-Log " Files scanned: $($files.Count)" -Color Cyan
    Write-Log " Total secrets found: $($allFindings.Count)" -Color Cyan
    Write-Log " Blocking: $($summary.BlockingSecrets)" -Color Cyan
    Write-Log " Warnings: $($summary.WarningSecrets)" -Color Cyan
    
    foreach ($report in $reports.GetEnumerator()) {
        Write-Log " $($report.Key) report: $($report.Value)" -Color Cyan
    }
...
# ===== MAIN EXECUTION =====
try {
    Write-Log " Agent Exo-Suit V4.0 'PERFECTION' - Secret Scanner Starting..." -Color Cyan
    ...
    $findings = if ($results.Findings) { $results.Findings } else { @() }
    $blockingSecrets = if ($findings) { $findings | Where-Object { $_.Severity -eq 'BLOCK' } } else { @() }
    
    if ($blockingSecrets -and $blockingSecrets.Count -gt 0 -or $Strict) {
        Write-Log " Blocking secrets found or strict mode enabled" -Color Red
        exit 3
    } elseif ($findings -and $findings.Count -gt 0) {
        Write-Log " Warning secrets found" -Color Yellow
        exit 2
    } else {
        Write-Log " No secrets found - codebase is clean!" -Color Green
        exit 0
    }
...
Write-Log " V4.0 Secret Scanner ready for production deployment!" -Color Green
