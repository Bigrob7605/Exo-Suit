...
if (Test-Path $envFile) {
    ...
    Write-Host "⚠️ Dual-mode config not found, using defaults" -ForegroundColor Yellow
}
...
if ($GPU) {
    ...
    $deviceMode = "cpu"
    Write-Host "🖥️ Forcing CPU-only mode" -ForegroundColor Green
} elseif ($Hybrid) {
    $deviceMode = "hybrid"
    ...
    foreach ($package in $requiredPackages) {
        try {
            ...
            if ($result -eq "OK") {
                ...
                Write-Host "  ⚠️ $package (partial)" -ForegroundColor Yellow
            }
        } catch {
...
" 2>&1
    ...
    if ($gpuInfo -match "CUDA Available: True") {
        ...
        Write-Host "  ⚠️ GPU acceleration not available" -ForegroundColor Yellow
        $gpuAvailable = $false
    }
    ...
    Write-Host "  ⚠️ GPU detection failed" -ForegroundColor Yellow
    $gpuAvailable = $false
}

# CPU detection
Write-Host "🖥️ Detecting CPU capabilities..." -ForegroundColor Yellow
try {
    $cpuInfo = python -c "
...
" 2>&1
    ...
    Write-Host "  ⚠️ CPU detection failed" -ForegroundColor Yellow
}
