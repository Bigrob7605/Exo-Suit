...
// Compression statistics with ZSTD comparison
#[derive(Debug, Clone)]
struct CompressionStats {
    ...
    zstd_ratio: Option<f64>, // NEW: ZSTD comparison
}
...
impl OptimizedPatternRecognitionEngine {
    ...
    fn analyze_file(&self, file_path: &Path) -> Result<CompressionStats, Box<dyn std::error::Error>> {
        ...
        // NEW: Get ZSTD baseline for comparison
        let zstd_ratio = self.get_zstd_baseline(file_path)?;
    ...
    /// NEW: Get ZSTD baseline for comparison
    fn get_zstd_baseline(&self, file_path: &Path) -> Result<Option<f64>, Box<dyn std::error::Error>> {
        // Try to use zstd command if available
        let output = Command::new("zstd")
            .arg("-19") // Maximum compression
            .arg("-q")  // Quiet mode
        ...
        match output {
            Ok(output) if output.status.success() => {
                let zstd_size = output.stdout.len();
                let original_size = fs::metadata(file_path)?.len() as usize;
                let ratio = zstd_size as f64 / original_size as f64;
                Ok(Some(ratio))
            }
            _ => Ok(None), // ZSTD not available
        }
    }
}
...
// Main test function
fn main() -> io::Result<()> {
    println!("🚀 OPTIMIZED Visual Enhanced Pattern Recognition Test");
    println!("==================================================");
    ...
    for file_path in files {
        match engine.analyze_file(&file_path) {
            Ok(stats) => {
                ...
                // Show file results with ZSTD comparison
                ...
                if let Some(zstd_ratio) = stats.zstd_ratio {
                    let zstd_percent = 100.0 * (1.0 - zstd_ratio);
                    println!("   🏆 ZSTD-19 baseline: {:.1}% reduction (ratio: {:.2}x)", 
                        zstd_percent, zstd_ratio);
                    
                    // Show how we compare
                    if stats.ratio > zstd_percent {
                        println!("   🎯 We're {:.1}% better than ZSTD!", 
                            stats.ratio - zstd_percent);
                    } else {
                        println!("   📈 ZSTD is {:.1}% better than us", 
                            zstd_percent - stats.ratio);
                    }
                }
            }
            Err(e) => {
    ...
    if !all_stats.is_empty() {
        ...
        // Show ZSTD comparison if available
        let zstd_stats: Vec<_> = all_stats.iter().filter_map(|s| s.zstd_ratio).collect();
        if !zstd_stats.is_empty() {
            let avg_zstd_ratio: f64 = zstd_stats.iter().sum::<f64>() / zstd_stats.len() as f64;
            let avg_zstd_percent = 100.0 * (1.0 - avg_zstd_ratio);
            println!("🏆 ZSTD-19 average: {:.1}% reduction", avg_zstd_percent);
            
            if overall_ratio > avg_zstd_percent {
                println!("🎉 SUCCESS: We beat ZSTD by {:.1}%!", 
                    overall_ratio - avg_zstd_percent);
            } else {
                println!("📈 ZSTD beats us by {:.1}% - room for improvement!", 
                    avg_zstd_percent - overall_ratio);
            }
        }
    }
