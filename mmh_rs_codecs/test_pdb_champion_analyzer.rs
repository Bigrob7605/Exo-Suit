// Improved PDB Champion Analyzer - Refactored for Performance and Maintainability
...
pub struct PDBAnalyzerConfig {
    ...
    pub sample_size_mb: usize, // For large file sampling
}
...
impl Default for PDBAnalyzerConfig {
    fn default() -> Self {
        Self {
            ...
            threshold: 0.03, // Optimized for PDB files
            max_file_size_mb: 500,
            target_compression_potential: 8000.0,
            sample_size_mb: 10, // Sample 10MB for large files
        }
    }
}
...
/// PDB-specific metrics (simplified and more accurate)
#[derive(Debug, Clone)]
...
impl PatternAnalyzer {
    ...
    fn analyze_patterns(&self, data: &[u8]) -> CompressionMetrics {
        ...
        // Use more efficient pattern detection
    ...
    /// Get sample data for analysis (avoid loading entire large files into memory)
    fn get_sample_data<'a>(&self, data: &'a [u8]) -> &'a [u8] {
        ...
        if data.len() <= max_sample_size {
            ...
            // For now, just return the first chunk (could be optimized later)
    ...
    fn detect_patterns_efficiently(&self, data: &[u8]) -> (usize, f64) {
        ...
        // Focus on the most promising pattern lengths
    ...
    fn estimate_compression_ratio(&self, potential: f64, entropy: f64) -> f64 {
        let entropy_factor = (8.0 - entropy) / 8.0; // Lower entropy = better compression
...
impl PDBParser {
    ...
    fn parse_header(data: &[u8]) -> PDBMetrics {
        ...
        // Estimate data sizes using header information
    ...
    fn estimate_symbol_size(data: &[u8], page_size: u32) -> usize {
        // More accurate estimation based on actual PDB structure
        let symbol_stream_size = page_size as usize * 2; // Rough estimate
        symbol_stream_size.min(data.len() / 4)
    }

    fn estimate_debug_size(data: &[u8]) -> usize {
        // Count debug-related patterns
        let mut debug_size = 0;
        let patterns = vec![b"DEBUG", b"RSDS", b"NB10", b"PDB"];
        
        for pattern in &patterns {
            for window in data.windows(pattern.len()) {
                if window == *pattern {
                    debug_size += 1024; // Estimate 1KB per debug marker
                }
            }
        }
    ...
    fn estimate_string_size(data: &[u8]) -> usize {
        ...
        for &byte in data.iter().take(data.len().min(100000)) { // Sample first 100KB
        ...
        string_bytes * 10 // Extrapolate from sample
    }

    fn has_type_information(data: &[u8]) -> bool {
        let type_markers = vec![b"TYPE", b"LF_", b"TPI"];
        type_markers.iter().any(|marker| {
...
impl PDBChampionAnalyzer {
    ...
    pub fn discover_pdb_files(&self, root_path: &str) -> Result<Vec<PDBFileInfo>, Box<dyn std::error::Error>> {
        ...
        // Sort by estimated potential (size-based heuristic)
        ...
        for entry in fs::read_dir(dir)? {
            ...
            if path.is_file() && self.is_pdb_file(&path) {
                if let Ok(metadata) = fs::metadata(&path) {
                    ...
                    // Estimate potential based on size (larger PDB files often have more compression potential)
                ...
                // Skip common non-PDB directories
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🏆 PDB CHAMPION ANALYZER - Improved Version");
    println!("{}", "=".repeat(70));
    ...
    // Analyze files (limit to reasonable number for demo)
